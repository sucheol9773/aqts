> **아카이브 문서 — 수정 금지**
>
> 본 문서는 **2026-04-17 Phase 1 마이그레이션 이전의 `CLAUDE.md` 원본 스냅샷**(284줄)입니다.
> 모든 규칙은 `agent_docs/development-policies.md` 로 원문 이반되었으며, 향후 규칙 변경은 **해당 문서만** 갱신합니다.
> 본 파일은 마이그레이션 충실도 대조 용도로만 참조하시고, **절대 수정하지 마십시오**.
>
> - 원본 파일명: `CLAUDE.md.bak`
> - 스냅샷 시점: 2026-04-16 (Phase 1 마이그레이션 직전)
> - 리드 전용 변경 영역: `agent_docs/governance.md §2.5` 참조
...
## 인증(authn) ≠ 인가(authz) 분리 원칙 (RBAC Wiring Rule)
...
## 공급망 보안 검증 규칙 (Supply-Chain Security Rule)
...
## 알림 파이프라인 Wiring Rule (Alerting Pipeline Wiring Rule)
...
## SSH Heredoc 에서 비대화형 원격 명령 작성 규칙
