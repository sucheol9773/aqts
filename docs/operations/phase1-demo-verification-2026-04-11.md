...
#### 배포 후 실측 결과 (2026-04-15 09:39 UTC, sha-e7b5187)

| 검증 항목 | 기대 | 실측 | 판정 |
|---|---|---|---|
| 이미지 digest (backend) | `ghcr.io/sucheol9773/aqts-backend:sha-e7b5187` | `sha256:2be094b76d4a…@sha-e7b5187` | ✓ |
| 이미지 digest (scheduler) | 동일 digest (backend 과 일치) | `sha256:2be094b76d4a…@sha-e7b5187` | ✓ |
| `PortfolioLedger hydrated` literal `%d` 해소 | `positions=N` (literal 아님) | `PortfolioLedger hydrated from DB (positions=0)` @ `main:lifespan:209` | ✓ |
| backend 전체 literal `%[dsfxi]` 잔존 | 0 건 | `✓ no literal % posarg in logs` | ✓ |
| scheduler 전체 literal `%[dsfxi]` 잔존 | 0 건 | `✓ no literal % posarg in logs` | ✓ |

`main:lifespan` 의 소스 라인이 207 → 209 로 시프트된 것은 §10.15 수정 시점에 추가한 "loguru % posarg 해석 불가" 주석 2 줄 때문이며 의도된 결과다.

`positions=0` 은 SqlPortfolioLedgerRepository hydrate 결과로, **내부 ledger 는 비어있고 KIS DEMO broker 에는 여전히 13 종목이 존재**하는 상태를 의미한다. §10.15 표 #5 (`core/reconciliation_runner.py:110` mismatch CRITICAL) 가 경고했던 "내일 MIDDAY_CHECK 에서 실제로 mismatch 발화 가능" 조건이 그대로 유지되고 있다. 11:30 KST 이전에 HTS flatten 을 완료하지 않으면 `Reconciliation mismatch detected: count=13 diff_abs=... mismatches=[...]` 로그가 f-string 으로 정확히 찍히고 TradingGuard 가 kill switch 로 주문 경로를 차단한다 — 이번 수정 덕분에 그 CRITICAL 로그의 상세는 literal 로 버려지지 않고 진단 정보 전량이 관측 레이어에 도달한다.


### 10.17 TradingGuard kill switch 수동 해제 HTTP 엔드포인트 도입 (2026-04-15)

#### 배경

§10.15 · §10.16 에서 관측 레이어(loguru → docker logs) 의 silent miss 를 해소한 뒤, 2026-04-16 11:30 KST MIDDAY_CHECK 의 두 경로를 비교 평가했다.

- **Path A (no-flatten + 수동 해제 경로 공란)**: mismatch CRITICAL → TradingGuard kill switch 활성 → 해제 경로가 문서/코드 어디에도 노출돼 있지 않음 → 운영자가 컨테이너에 직접 들어가 `TradingGuard._guard_instance.deactivate_kill_switch()` 를 파이썬 REPL 로 호출해야 하는 상태. 감사는 호출자가 수동으로 남기지 않으면 비어 있음.
- **Path B (사전 flatten)**: mismatch 자체가 발생하지 않아 kill switch 경로를 실측할 기회를 놓친다.

핵심 결정: **"개발 단계에서 당일 운영 중단의 손실은 0 으로 간주할 수 있다"** (사용자 확인) 는 전제 하에서 Path A 를 택하는 것이 가치가 크지만, Path A 는 **kill switch 수동 해제가 감사 선행 조건을 만족한 상태로 형식화돼 있어야** 실제로 안전하다. 현재는 다음 세 결손이 있다:

1. **엔드포인트 부재** — `backend/api/routes/` 내 kill switch 관련 라우트가 0 건.
2. **문서 모순** — `docs/operations/trading-halt-policy.md` v1.0 이 존재하지 않는 `/api/system/halt`, `/api/system/resume` 를 기술하고, `PipelineStateMachine.HALTED` 와 `TradingGuard.kill_switch_on` 을 같은 층위로 섞어 설명.
3. **해제 후 재hydrate 트리거 미정의** — kill switch 해제 후 내부 ledger 가 최신 DB 상태와 자동 동기화되는 경로가 없어, 해제 직후 pipeline cycle 이 돌면 stale cache 로 다시 mismatch 를 유발할 수 있는 silent miss 회귀 후보.

이 세 결손을 해소하지 않고 Path A 를 집행하면 내일 11:30 KST 에 "수동 해제가 필요한데 해제 절차가 비표준 REPL" 인 상황에 진입한다. 운영 가치가 감사 손실·회귀 위험과 섞여 평가 품질을 떨어뜨린다.

#### 구현: `/api/system/kill-switch/*` 두 엔드포인트

| 엔드포인트 | 메서드 | 권한 | 역할 |
|---|---|---|---|
| `/api/system/kill-switch/status` | GET | `require_viewer` | `kill_switch_on`, `kill_switch_reason` snapshot 반환 |
| `/api/system/kill-switch/deactivate` | POST | `require_admin` | 감사 선행 → 해제 → ledger 재hydrate |

`POST /kill-switch/deactivate` 동작 사양:

1. **Confirm gate**: body `confirm=false` → 400 `CONFIRM_REQUIRED` (Pydantic `reason` 은 10자 이상 제약).
2. **Audit fail-closed**: `AuditLogger.log_strict(action_type="KILL_SWITCH_DEACTIVATE", before_state={"kill_switch_on": <prev>, "kill_switch_reason": <prev>}, after_state={"kill_switch_on": false, "kill_switch_reason": ""}, metadata={"username": <admin_id>, "reason": <body.reason>})` 선행. `AuditWriteFailure` raise 시 503 `AUDIT_UNAVAILABLE` + kill switch **유지**.
3. **State transition**: 감사 성공 후 `get_trading_guard().deactivate_kill_switch()` → gauge `aqts_trading_guard_kill_switch_active` → 0.
4. **Ledger re-hydrate**: `get_portfolio_ledger()` 의 `repository` 가 None 이 아니면 `await ledger.hydrate()` 호출. 실패는 warning 로그만 남기고 `ledger_rehydrated=False` 로 응답 (해제 자체는 유효).
5. **Response**: `was_on`, `previous_reason`, `deactivated_at` (ISO-8601 KST), `ledger_rehydrated`, `ledger_positions_count`, `operator`.
...
curl -sS -X POST \
  ...
  -d '{"reason":"DEMO 환경 MIDDAY_CHECK mismatch 원인 확인 후 수동 해제","confirm":true}' \
  http://<backend>/api/system/kill-switch/deactivate | jq .
```

#### 파일 변경

- `backend/api/routes/system.py` — 신규 Pydantic 모델 3개 (`KillSwitchDeactivateRequest`, `KillSwitchStatus`, `KillSwitchDeactivateResponse`) + 두 라우트 핸들러. 기존 `require_admin` · `require_viewer` · `AuditLogger` · `AuditWriteFailure` · `get_portfolio_ledger` · `get_trading_guard` · `HTTPException` · `BaseModel` · `Field` import 추가.
- `backend/tests/test_kill_switch_routes.py` — 신규 9 테스트 (6 클래스):
  - `TestKillSwitchStatus` (2): fresh/activated 상태 snapshot 반영.
  - `TestKillSwitchDeactivateConfirmGate` (2): `confirm=false` → 400 + 상태 유지 / reason 10자 미만 → `ValidationError`.
  - `TestKillSwitchDeactivateHappyPath` (2): 감사 선행 → 해제 → hydrate 순서 · 부작용 검증 + 이미 off 인 경우 idempotent.
  - `TestKillSwitchDeactivateFailClosed` (1): `AuditWriteFailure` → 503 + kill switch 유지 + ledger re-hydrate 미호출.
  - `TestKillSwitchDeactivateRehydrateResilience` (1): hydrate 실패해도 해제는 완료 + `ledger_rehydrated=False`.
  - `TestKillSwitchGaugeWiring` (1): Prometheus gauge 1 → 0 전이.
- `docs/operations/trading-halt-policy.md` — **v1.1 로 개정**. §0 개정 이력 / §1 두 계층 구분 표 / §2.2 TradingGuard 자동 차단 트리거 / §3.5 수동 해제 엔드포인트 절차 / §4 재개 절차 이원화 / §6 코드 연동 포인트 갱신.
- `docs/security/security-integrity-roadmap.md` — §9 에 P0-5 후속 항목 추가 (kill switch 해제 절차 형식화 완료).

#### Wiring 검증 — "정의 ≠ 적용" 전수 확인

`scripts/check_rbac_coverage.py` 는 AST 기반으로 `@router.<method>` 데코레이터 전수를 스캔해 `Depends(require_*)` 누락을 차단한다. 신규 두 라우트는 코드 추가만으로 자동 검증되며, viewer 토큰으로 `POST /kill-switch/deactivate` 호출 시 403 이 반환되는지는 `tests/test_rbac_routes.py` 가 자동으로 커버한다(매트릭스 자동 추가). 별도 수동 등록 불필요.

핵심 wiring 결손 후보와 방어:

| 결손 후보 | 방어 수단 |
|---|---|
| RBAC 가드 누락 | `check_rbac_coverage.py` AST 정적 검사 (0 errors 강제) |
| 감사 fail-closed 미적용 (해제 선집행) | `test_audit_failure_refuses_deactivation` 통합 테스트 |
| ledger 재hydrate 트리거 부재 | `test_deactivate_fires_audit_then_clears_state` 에서 `hydrate.assert_awaited_once()` |
| hydrate 실패로 해제 자체가 막히는 역효과 | `test_ledger_rehydrate_failure_does_not_block_deactivation` |
| Prometheus gauge 미전이 | `test_gauge_drops_to_zero_after_deactivation` |
| confirm 우회로 실수 해제 | `test_confirm_false_raises_400` + Pydantic `min_length=10` |

#### 내일 (2026-04-16 11:30 KST) MIDDAY_CHECK 집행 계획

Path A 를 집행한다. 기대 시퀀스:

1. `09:00 KST` 시장 개장 후 첫 pipeline cycle 이 `PortfolioLedger.hydrate()` 완료 상태(`positions=0`) 에서 출발.
2. `11:30 KST` `midday_mismatch_check` 스케줄러가 HTS 13종목 vs ledger 0종목 비교 → `Reconciliation mismatch detected: count=13 diff_abs=... mismatches=[...]` CRITICAL 로그 (§10.15 수정으로 literal `%` 손실 없음).
3. `TradingGuard` 가 kill switch 활성화: `activate_kill_switch("MIDDAY_CHECK mismatch — count=13")`. Prometheus gauge `aqts_trading_guard_kill_switch_active` → 1. Telegram CRITICAL 알림 발송.
4. 이후 모든 주문 요청은 `TradingGuardBlocked` 로 거부 (f-string 으로 진단 정보 전량 로그에 존재).
5. 운영자 (admin 토큰) 는 `/api/system/kill-switch/status` 로 상태 확인 → 원인 분석 (HTS 에 존재하지만 ledger 에 없는 13종목의 출처 추적) → `/api/system/kill-switch/deactivate` 로 감사 선행 해제 → 해제 응답 body 의 `ledger_rehydrated=true`, `ledger_positions_count=N` 을 확인하여 DB → cache 동기화가 실제로 이루어졌는지 검증.
6. 해제 직후 다음 pipeline cycle 에서 stale cache 가 아닌 최신 positions 로 재판정되는지 확인 (silent miss 회귀 방지).

#### 기대 관측 — 성공 판정 조건

- mismatch CRITICAL 로그에 `count=13`, `mismatches=[...]` 가 literal `%[dsfx]` 없이 나타난다 (§10.15, §10.16 수정 검증).
- `/kill-switch/deactivate` 응답: `was_on=true`, `previous_reason` 에 "MIDDAY_CHECK mismatch" 포함, `ledger_rehydrated=true`, `ledger_positions_count >= 0`, `operator=<admin id>`.
- `audit_logs` 테이블에 `KILL_SWITCH_DEACTIVATE` 행이 선행 기록 (해제 전 타임스탬프).
- 해제 후 Prometheus gauge `aqts_trading_guard_kill_switch_active == 0`.
- 해제 후 다음 pipeline cycle 이 정상 진행되고, 재발생한 mismatch 가 있다면 그것은 **새로운 원인**이며 stale cache 에 의한 가짜 회귀가 아님.

#### 회고

§10.15, §10.16 이 관측 레이어의 silent miss (literal `%` / regex 커버리지) 를 해소했다면, §10.17 은 **운영 절차의 silent miss** — 해제 경로가 코드·문서에 존재하지 않아 운영자가 REPL 로 감사 없이 해제하게 되는 경로 — 를 해소한다. RBAC Wiring Rule / 공급망 Wiring Rule / 알림 파이프라인 Wiring Rule 과 동일한 **"정의 ≠ 적용"** 원칙의 운영 절차 도메인 확장이다.
