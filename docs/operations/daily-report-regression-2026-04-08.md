...
### 검증 결과 (2026-04-09, `6c34ebe` 배포)

`da59ada` (C2b) + `6c34ebe` (verify step revision 교차 체크) 가 포함된 사이클의 Post-deploy 로그에서 네 지점이 모두 기대대로 출력되었다.

- `Deployed commit: 6c34ebe73945 (from container label)` — verify step 이 `git rev-parse` 가 아닌 컨테이너 `org.opencontainers.image.revision` 라벨에서 읽는 경로가 실제로 동작.
- `Server git HEAD: 6c34ebe73945` — 컨테이너 revision 과 서버 HEAD 가 일치, server drift 분기 통과.
- `[C2a] digest match: sha256:cd03d6dd56bb…` — backend ↔ scheduler digest 동일성 유지.
- `[C2b] revision label matches server HEAD: 6c34ebe73945…` — 신규 교차 체크가 실제 pass 로그를 찍었다.

Step 5c (alembic upgrade) 로그가 `Will assume transactional DDL.` 에서 끊기지 않고 Step 5d / 5e / 6 까지 모두 출력되었다. §4.7/§4.8 에서 관측된 heredoc stdin 소진 경로는 `43b388b` 의 `-T + </dev/null` 과 `5cacdcf` 의 정적 가드로 이중 차단되어 있음이 재확인되었다.

## 4.11 heredoc 전수 감사 + 하위 스크립트 fd 0 상속 차단 (Rule 5)

§4.7/§4.8/§4.9/§4.10 의 대응 이후 "같은 은폐 경로가 다른 형태로 어딘가에 더 있지 않은가" 를 확인하기 위해 `.github/workflows/*.yml` 과 `scripts/**/*.sh` 전수 감사를 수행했다. 감사 결과와 대응은 별도 리포트 `docs/operations/cd-heredoc-stdin-audit-2026-04-09.md` 에 정리했으며, 요점은 다음과 같다.

**결론**: 감사 시점 기준 active 취약점은 없다. 그러나 **잠재 갭 1 건** 이 식별됐다 — `cd.yml:361` 의 `bash scripts/post_deploy_smoke.sh` 호출이 VERIFY_SCRIPT heredoc 내부에서 `</dev/null` 없이 실행되고 있었다. 자식 bash 가 부모 heredoc 의 fd 0 을 상속하므로, smoke 스크립트 내부에 장래 `docker exec -i` 한 줄만 추가되면 VERIFY_SCRIPT 의 잔여 라인(`echo "✅ All verification checks passed"`) 이 조용히 소진되고 §4.7 패턴이 재발한다.

**대응**: 이중 방어선.

- **Layer A (호출 지점 격리)**: `cd.yml:361` 을 `bash scripts/post_deploy_smoke.sh </dev/null` 로 수정. 자식 bash 의 fd 0 을 /dev/null 에 고정해 상속 사슬을 즉시 끊는다.
- **Layer B (정적 가드 확장, Rule 5)**: `check_cd_stdin_guard.py` 에 Rule 5 를 추가. heredoc 내부에서 `bash X.sh` / `sh X.sh` / `./X.sh` 호출 시 같은 논리 라인에 `</dev/null`, `< FILE`, `<<<`, 또는 파이프 입력이 없으면 ERROR. "heredoc 내부에서 호출되는 모든 하위 shell 스크립트는 stdin 을 명시적으로 격리해야 한다" 를 기계적으로 강제한다.

회귀 테스트 `TestRule5InheritedScriptInvocation` 8 개 케이스(flag 해야 할 3 가지 패턴 + 통과해야 할 3 가지 격리 방식 + 오탐 방지 2 가지)가 이 규칙을 고정한다.

**원칙의 일반화**: §4.7 → §4.8 의 교훈("플래그 중심 규칙 ≠ 기본값 중심 규칙") 에 이어, §4.11 은 한 단계 더 일반화한다: 규칙은 "`-i` 플래그" 나 "`-T` 플래그" 가 아니라 **"자식 프로세스가 부모 fd 0 을 읽을 수 있는가"** 로 표현되어야 한다. Rule 1~4 가 "어떤 명령에 어떤 플래그" 로 시작했다면, Rule 5 는 "fd 0 상속 관계" 자체를 끊는 쪽으로 이동한 것이다. 이 방향성이 앞으로 추가될 모든 규칙의 기본 축이다.

## 종결 상태

§4.7 ~ §4.11 의 모든 대응이 실측(§4.10) + 감사(§4.11) 로 검증됐다. 이후 관찰 창(2026-04-09 ~ 2026-04-16, 1주) 동안 동일 패턴(Step 5c 로그 절단, 컨테이너-서버 SHA drift, C2a 위양성, heredoc fd 0 상속으로 인한 은폐) 재발 0건이 유지되면 본 회귀는 공식 종결한다. 관찰 창 중 Telegram 경보가 해제된 상태를 유지하는지도 함께 추적한다.
...
- 본 문서 §4.11 의 감사 + Rule 5: `fix(ci): heredoc 내부 하위 스크립트 fd 0 상속 차단 (Rule 5) + 전수 감사 보고`
- 감사 보고 별도 문서: `docs/operations/cd-heredoc-stdin-audit-2026-04-09.md`
