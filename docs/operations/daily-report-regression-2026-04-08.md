...
§4.7 ~ §4.10 의 모든 대응이 실측으로 검증됐다. 이후 관찰 창(2026-04-09 ~ 2026-04-16, 1주) 동안 동일 패턴(Step 5c 로그 절단, 컨테이너-서버 SHA drift, C2a 위양성) 재발 0건이 유지되면 본 회귀는 공식 종결한다. 관찰 창 중 Telegram 경보가 해제된 상태를 유지하는지도 함께 추적한다.
