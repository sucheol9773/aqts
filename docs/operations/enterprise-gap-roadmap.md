...
### P4 (P3 병행 또는 후속) — "강화학습/학습형 전환"

2026-04-12 코드베이스 분석 결과, RL 인프라(환경/트레이너/레지스트리/추론 서비스)가 이미 구현되어 있으나, 실제 학습과 라이브 전환에 필요한 데이터 축적·검증 기준·운영 절차가 부재하다. P4는 기존 RL 코드를 **"구현됨"에서 "운영됨"으로 승격**시키는 단계이다.

#### 현재 RL 인프라 상태

| 컴포넌트 | 위치 | 상태 |
|----------|------|------|
| 단일 자산 RL 환경 (Gymnasium) | `core/rl/environment.py` | ✅ 구현 |
| 멀티에셋 RL 환경 | `core/rl/multi_asset_env.py` | ✅ 구현 |
| PPO/SAC 트레이너 | `core/rl/trainer.py` | ✅ Stable-Baselines3 |
| 모델 레지스트리 (챔피언 선택) | `core/rl/model_registry.py` | ✅ 구현 |
| 실시간 추론 서비스 | `core/rl/inference.py` | ✅ 섀도우 모드 |
| 앙상블 블렌딩 | `core/rl/inference.py` | ✅ RL 40% : 앙상블 60% |
| Optuna 하이퍼파라미터 튜닝 | `core/rl/hyperopt_rl.py` | ✅ 구현 |
| Walk-forward 평가 | `core/rl/trainer.py` | ✅ 70/15/15 분할 |

#### 블록 J: 데이터 축적 + 관측 공간 확장

**시점**: Phase 1 DEMO 검증 통과 직후 시작 (데이터는 자동으로 쌓임)

**실행 항목**:

1. **데이터 축적 기준선 확보**
   - RL 훈련에 최소 312일 (60일 lookback + 252 거래일) 분량 필요
   - DEMO 모드 데이터 수집 시작일: 2026-04-13 (Phase 1 첫 거래일)
   - 최소 훈련 가능 시점: 2027-07 경 (자연 축적 기준)
   - 가속 옵션: 과거 OHLCV 데이터를 외부 소스(KRX, Yahoo Finance)에서 일괄 적재

2. **관측 공간(Observation Space) 확장**
   - 현재: 11차원 (시장 피처 + 3개 전략 시그널 + 포트폴리오 상태)
   - 추가 대상: Claude 센티먼트 점수, 시장 레짐 라벨, ECOS/FRED 경제지표, 거래량 프로파일
   - `core/rl/environment.py:_get_observation()` 수정
   - 차원 증가에 따른 네트워크 구조 조정 (PPO policy net hidden layers)

3. **데이터 파이프라인 검증**
   - 일별 시그널/센티먼트/레짐 데이터가 누락 없이 저장되는지 모니터링
   - 결측치 처리 정책 수립 (보간/제거/플래그)
   - `scripts/validate_rl_data_quality.py` 스크립트 작성

| 항목 | 값 |
|---|---|
| 상태 | ⏳ 데이터 축적 대기 (2026-04-13 시작) |
| 예상 산출물 | 확장된 observation space, 데이터 품질 검증 스크립트, 과거 데이터 적재 스크립트 (선택) |

---

#### 블록 K: 보상 함수 고도화 + 오프라인 훈련

**시점**: 데이터 3개월 축적 후 (또는 과거 데이터 적재 완료 후)

**실행 항목**:

1. **보상 함수 개선**
   - 현재: `일일 P&L - risk_penalty × drawdown - cost_penalty × 거래비용`
   - 개선안 A: Sortino 기반 (하방 변동성만 페널티)
   - 개선안 B: CVaR(Conditional Value at Risk) 기반 (꼬리 리스크 민감)
   - 백테스트 엔진으로 보상 함수별 성과 비교 실험

2. **오프라인 훈련 파이프라인**
   - Walk-forward 훈련: 학습→검증→테스트 롤링 윈도우
   - PPO vs SAC 성과 비교 (OOS Sharpe 기준)
   - 과적합 방지: 정규화, 조기 종료, 앙상블 내 RL 정책 다중화
   - 훈련 결과를 모델 레지스트리에 자동 등록

3. **훈련 환경 결정**
   - 소규모 훈련 (단일 자산, <100K timesteps): 현재 서버 (vCPU 2, 유휴 95%)에서 가능
   - 본격 하이퍼파라미터 탐색: GCP spot VM 또는 Colab Pro 활용
   - Optuna 분산 탐색 시 별도 `training` compose profile 또는 클라우드 러너

| 항목 | 값 |
|---|---|
| 상태 | ⏳ 대기 |
| 선행 조건 | 블록 J 데이터 축적 (최소 3개월 또는 과거 데이터 적재) |
| 예상 산출물 | 개선된 보상 함수, 훈련된 PPO/SAC 모델, OOS 성과 비교 리포트 |

---

#### 블록 L: 섀도우 검증 + 라이브 전환

**시점**: 오프라인 훈련 완료 후 1~2개월 섀도우 운영

**실행 항목**:

1. **섀도우 모드 검증 기준 수립**
   - RL 시그널 vs 기존 앙상블 시그널 일별 비교 대시보드
   - 전환 기준: OOS Sharpe가 기존 앙상블 대비 N% 이상, 최대 낙폭이 기존 이하, 최소 40거래일 연속 관측
   - 실패 기준: 연속 10거래일 이상 앙상블 대비 열위 → 자동 RL 비활성화

2. **점진적 라이브 전환**
   - 단계 1: RL 40% : 앙상블 60% (현재 기본값, 섀도우에서 라이브로 전환)
   - 단계 2: RL 60% : 앙상블 40% (1개월 관측 후)
   - 단계 3: RL 80% : 앙상블 20% (2개월 관측 후)
   - 각 단계 전환 시 `core/rl/inference.py` 블렌딩 비율 조정 + 알림

3. **안전장치**
   - RL 모델 드리프트 탐지: 일별 action 분포 변화 모니터링
   - 긴급 롤백: RL 비율을 0%로 즉시 전환하는 운영 명령 (API 또는 환경변수)
   - 레짐 전환 시 자동 재훈련 트리거 기준 수립

4. **메타 학습 (장기 목표)**
   - RL 에이전트가 앙상블 가중치 자체를 동적으로 조정하는 구조
   - 월별 리밸런싱 → RL 기반 실시간 리밸런싱으로 전환
   - Hierarchical RL: 상위(포트폴리오 배분) + 하위(종목 타이밍)

| 항목 | 값 |
|---|---|
| 상태 | ⏳ 대기 |
| 선행 조건 | 블록 K 오프라인 훈련 완료 + OOS 검증 통과 |
| 예상 산출물 | 섀도우 비교 대시보드, 전환 기준 문서, 블렌딩 비율 자동 조정 코드, `docs/ml/rl-live-transition.md` |
...
```
2026-04        Phase 1 DEMO 검증 + 데이터 수집 시작
  │
  ├─ 블록 J: 관측 공간 확장 (데이터 축적과 병행, 1~2주)
  │
2026-07        데이터 3개월 축적
  │
  ├─ 블록 K: 오프라인 훈련 + 보상 함수 실험 (2~4주)
  │
2026-08~09     섀도우 모드 운영
  │
  ├─ 블록 L: 섀도우 검증 + 점진적 라이브 전환 (1~2개월)
  │
2026-10~       RL 블렌딩 라이브 운영
```

과거 데이터를 외부 소스에서 적재하면 블록 K를 2~3개월 앞당길 수 있다.
...
P4(블록 J~L)는 **RL/학습형 전환** 단계로, P3와 병행 가능하다. 데이터 축적(J)은 Phase 1 검증 직후 자동으로 시작되며, 3개월 후 오프라인 훈련(K)을 거쳐 섀도우 검증 → 점진적 라이브 전환(L)으로 이어진다. RL 인프라(환경/트레이너/레지스트리/추론)는 이미 구현되어 있으므로, 핵심 과제는 "충분한 데이터 확보"와 "전환 기준의 정량화"이다.
...
## 부록 B. P3 + P4 블록 의존성 그래프

```
[P3: 운영 성숙도]                          [P4: RL/학습형 전환]

블록 A (Scheduler 분리)                    블록 J (데이터 축적 + 관측 공간 확장)
  ├──→ 블록 B (TLS + 네트워크)                │  ← Phase 1 검증 직후 자동 시작
  │      └──→ 블록 E (시크릿 수명주기)         │
  │             └──→ 블록 F (Terraform IaC)   ├──→ 블록 K (보상 함수 + 오프라인 훈련)
  │                    ├──→ 블록 G (B/G 배포)  │     ← 데이터 3개월 후
  │                    └──→ 블록 H (장애 격리)  │
  └──→ 블록 C+D (DR 기준 + 드릴)              └──→ 블록 L (섀도우 검증 + 라이브 전환)
                    └──→ 블록 I (4-eyes)              ← 훈련 완료 + 40거래일 검증 후
                           ↑ P2 #9 RBAC

P3와 P4는 독립 트랙으로 병행 가능.
블록 J(데이터 축적)는 선행 조건 없이 즉시 시작.
```
...
## 부록 D. P4 추가 배경 (2026-04-12)

코드베이스 분석 결과 `core/rl/` 디렉터리에 RL 인프라(Gymnasium 환경, PPO/SAC 트레이너, 모델 레지스트리, 추론 서비스, Optuna 하이퍼파라미터 탐색)가 이미 구현되어 있음을 확인. 현재 앙상블 블렌딩(RL 40% : 앙상블 60%)이 섀도우 모드로 대기 중이나, 실제 훈련 데이터가 부재하여 학습이 수행되지 않은 상태. P4는 이 인프라를 실제 운영으로 승격시키는 단계이며, 데이터 축적이 핵심 선행 조건이므로 P3와 독립적으로 병행 가능하다.
