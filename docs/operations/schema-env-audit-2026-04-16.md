...
1. **`.env.example` 의 Category B (14건) 전량 추가**: pydantic 자동 read 8건 + 직접 read 6건. 특히 `DASHBOARD_PREVIOUS_SECRET_KEY` 는 JWT 키 롤링 운영에 필수이므로 최상위 우선.
2. **`.env.example` 의 Category C (9건) 전량 추가**: `IMAGE_TAG` 가 가장 중요 (운영 digest pin).
3. **RL 모듈 DB 키 통일 (Category D)**: `POSTGRES_*` → `DB_*` 로 이동, `DatabaseSettings` 재사용. 관련 테스트 추가.

### P2 (리팩터링, 별건 PR)

1. **Category A (10건) 제거**: 미사용 키 정리. `.env.example` 간결화.
