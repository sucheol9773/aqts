...
- [2026-04-15] P0-5 후속: TradingGuard kill switch 수동 해제 HTTP 엔드포인트 형식화 — 종전까지 `TradingGuard.deactivate_kill_switch()` 는 메서드로만 존재하고 HTTP/운영 경로가 없어 운영자가 컨테이너 REPL 로 감사 없이 해제하는 silent miss 경로가 잠복해 있었다. `docs/operations/trading-halt-policy.md` v1.0 이 존재하지 않는 `/api/system/halt`·`/resume` 를 기술한 상태였고 `PipelineStateMachine.HALTED` 와 `TradingGuard.kill_switch_on` 을 같은 층위로 섞어 설명하여 운영 절차가 두 개념을 분리하지 못했다. 본 커밋은 `backend/api/routes/system.py` 에 두 엔드포인트를 신설한다. `GET /api/system/kill-switch/status` (`require_viewer`) 는 `kill_switch_on` · `kill_switch_reason` snapshot 반환. `POST /api/system/kill-switch/deactivate` (`require_admin`) 는 (1) `confirm=false` → 400 `CONFIRM_REQUIRED` + Pydantic `reason min_length=10` 으로 실수 해제 차단, (2) `AuditLogger.log_strict(action_type="KILL_SWITCH_DEACTIVATE", before_state, after_state, metadata={"username", "reason"})` 선행 — 실패 시 503 `AUDIT_UNAVAILABLE` + kill switch **유지** (fail-closed, P0-4 동일 정책), (3) 감사 성공 후 `deactivate_kill_switch()` + Prometheus gauge `aqts_trading_guard_kill_switch_active` → 0, (4) `PortfolioLedger.hydrate()` 로 DB → cache 재동기화 (실패는 warning + `ledger_rehydrated=False`, 해제 자체는 유효 — hydrate 장애로 해제 자체가 막히는 역효과 차단). 응답 body: `was_on`, `previous_reason`, `deactivated_at` (KST ISO-8601), `ledger_rehydrated`, `ledger_positions_count`, `operator`. 통합 테스트 9건 추가 (`tests/test_kill_switch_routes.py`): status snapshot fresh/activated 반영(2), confirm gate(400 + state 유지 + reason min_length ValidationError) (2), happy path (감사 선행 → 해제 → hydrate 순서 + kwargs 검증 + response 필드 전수 검증), idempotent(이미 off 여도 200 + 감사 기록 + was_on=False), audit fail-closed(503 AUDIT_UNAVAILABLE + ledger re-hydrate 미호출 + guard 상태 유지), hydrate 예외 복원력(해제 자체는 완료 + ledger_rehydrated=False), Prometheus gauge 1→0 전이. RBAC 가드 wiring 은 `scripts/check_rbac_coverage.py` AST 검사기가 자동 차단하여 가드 누락 회귀 방지 (9위 RBAC 회고와 동일 방어선). 문서: `docs/operations/trading-halt-policy.md` v1.1 로 개정 (§0 개정 이력 / §1 두 계층 구분 표 / §2.2 TradingGuard 자동 차단 트리거 / §3.5 수동 해제 절차 + curl 예시 / §4 재개 절차 이원화 / §6 코드 연동 포인트 갱신), `docs/operations/phase1-demo-verification-2026-04-11.md` §10.17 로 설계 결정·Wiring 검증·내일(2026-04-16) MIDDAY_CHECK Path A 집행 계획 기록. 핵심: "정의 ≠ 적용" Wiring Rule 의 **운영 절차 도메인 확장** — RBAC/공급망/알림 파이프라인/정적 검사 커버리지와 동일한 원칙이 운영자 조작 경로에 적용됐다.
...
- [2026-04-09] P1-운영: alertmanager 템플릿 렌더링 + entrypoint sed wiring + amtool CI 게이트 — 도커 데스크탑에서 `aqts-alertmanager` 가 1분마다 재시작 루프를 도는 회귀를 발견. 로그는 `yaml: unmarshal errors: line 75/93/105: cannot unmarshal !!str ${TELEG... into int64` 를 반복했다. 직접 원인은 `monitoring/alertmanager/alertmanager.yml` 의 `chat_id` 필드에 `${TELEGRAM_CHAT_ID}` 가 리터럴로 들어 있었고, alertmanager 바이너리는 환경변수 네이티브 치환을 지원하지 않으므로 int64 unmarshal 이 실패한 것이다. **근본 원인 (정의 ≠ 적용)**: 도입 PR `888db64` 가 (a) `alertmanager.yml` 안에 `${VAR}` 플레이스홀더를 두고, (b) `.env` 에 `TELEGRAM_*` 변수를 정의했지만, (c) 컨테이너로 그 변수를 전달할 `environment:` 블록도, (d) 변수를 치환할 entrypoint/init 도, (e) 헬스체크/스모크도 어느 것도 wiring 되지 않은 상태로 머지됐다. 결과적으로 alertmanager 는 도입 시점부터 단 한 번도 정상 부팅한 적이 없었고, Prometheus 가 alert rule 을 평가해도 발화 경로가 사일런트 실패 상태였다. **수정**: (1) `monitoring/alertmanager/alertmanager.yml` → `alertmanager.yml.tmpl` 로 git mv (history 보존). 템플릿 헤더에 렌더링 모델·int64 주의사항 명시. (2) `docker-compose.yml` 의 `alertmanager` 서비스에 `environment.TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` 등록, 마운트를 `.tmpl` 로 변경, entrypoint 를 `/bin/sh -c` 로 override 하여 `: "${VAR:?...}"` fail-closed 가드 → busybox `sed` 로 `${TELEGRAM_BOT_TOKEN}`/`${TELEGRAM_CHAT_ID}` 를 `/tmp/alertmanager.yml` 에 렌더링 → `exec /bin/alertmanager --config.file=/tmp/alertmanager.yml`. compose `$$` 이스케이프 + sed single-quoted LHS / double-quoted RHS 분리 패턴으로 셸/compose 양 레이어 변수 충돌을 차단. (3) **CI 게이트 추가** (`.github/workflows/ci.yml` lint 잡): 더미 env 로 sed 렌더 → PyYAML 로 파싱 → `chat_id` int 타입 + `bot_token` non-empty str 검증 → 동일 렌더 결과를 `prom/alertmanager:v0.27.0` 의 `amtool check-config` 로 schema 검증. 두 단계 모두 통과해야 머지 가능. (4) `docs/operations/alerting.md` 신설 — 구성 개요, 템플릿 렌더링 모델, 환경변수 표, CI 게이트, 운영 절차(라우트 추가/헬스체크/토큰 회전), 수동 검증 절차, 본 회귀 회고와 재발 방지 체크리스트. **로컬 검증**: 더미 env 로 sed 렌더 → PyYAML 파싱 결과 `chat_id: 987654321 (int)`, `bot_token: '123456:DUMMY' (str)` 모두 통과. **재발 방지 패턴**: 본 회귀는 RBAC wiring/공급망 cosign verify/PortfolioLedger reconcile/cd alembic baseline 에 이은 다섯 번째 "정의 ≠ 적용" 사례. 신규 monitoring config 도입 시 적용해야 할 체크리스트(렌더링 경로 존재 / `environment:` 등록 / `.env.example` 등록 / CI 렌더 검증 / 첫 배포 헬스체크 직접 확인) 를 `alerting.md` §7.4 에 코드화했다.
- [2026-04-09] P1-운영: prometheus/alertmanager Docker healthcheck 추가 (관측 공백 차단) — 직전 alertmanager wiring 커밋(`5a22faf`)의 근본 교훈은 "한 번도 부팅한 적 없는데 아무도 몰랐다" 였다. CI 의 amtool 게이트는 config 문법 오류만 잡고, 런타임 장애(envsubst 경로 변경, 이미지 업그레이드, 볼륨 권한, 네트워크 이슈) 는 여전히 감지 경로가 없었다. **수정**: `docker-compose.yml` 의 `prometheus`/`alertmanager` 두 서비스에 `healthcheck:` (`wget -qO- http://localhost:<port>/-/ready || exit 1`, interval 15s, timeout 5s, retries 5, start_period 30s/20s) 추가. busybox wget 은 prom/* 베이스 이미지에 이미 포함되어 있으므로 추가 패키지 불필요. `alertmanager.depends_on.prometheus` 를 단순 순서 의존에서 `condition: service_healthy` 로 승격 — prometheus 가 부팅 실패한 상태에서 alertmanager 만 살아있는 비정상 조합을 compose 레벨에서 차단. `docs/operations/alerting.md` §5.2 에 `docker compose ps` healthy/unhealthy 지표와 `docker inspect --format '{{json .State.Health}}'` 상세 결과 조회 절차 추가, §7.4 체크리스트에 "Docker healthcheck 등록 여부" 한 줄 추가. **효과**: 이제 같은 종류의 회귀가 재발하면 `docker compose ps` 에 `(unhealthy)` 가 분 단위로 표시되고, CD 파이프라인의 "Wait for health" 단계에도 자동 포함되며, Docker Desktop UI 에서도 즉시 가시화된다.
- [2026-04-09] P1-운영: `IMAGE_NAMESPACE` 운영 `.env` 필수 키 문서화 (compose parse 차단 해소) — 직전 healthcheck 커밋(`cc4e88e`) 을 서버에서 검증하던 중 `docker compose ps prometheus alertmanager` 가 `error while interpolating services.backend.image: required variable IMAGE_NAMESPACE is missing a value` 로 차단되는 것을 직접 관찰했다. compose 는 parse 단계에서 모든 서비스의 `${VAR:?...}` interpolation 을 한 번에 검사하므로, `backend`/`scheduler` 의 `${IMAGE_NAMESPACE:?IMAGE_NAMESPACE is required}` 가드가 `prometheus`/`alertmanager` 만 타겟한 명령까지 전부 실패시킨다. `docker inspect --format '{{.State.Health.Status}}'` 로 우회 검증은 가능하지만, 이는 방금 추가한 healthcheck 를 표준 운영 흐름으로 관찰할 수 없게 만들어 관측 가치를 퇴색시킨다. **수정 범위** (one-cause-one-commit): (1) `.env.example` Docker 설정 섹션에 `IMAGE_NAMESPACE=your_github_owner_lowercase` 키 + 주석 추가 — 필수 키임을 명시하고 빈 값 시 compose 전체 중단 양상을 설명. (2) `docs/operations/docker-setup-guide.md` §3.2 의 환경변수 목록에 "컨테이너 이미지 레지스트리 (운영 서버 필수)" 블록 추가, `${IMAGE_NAMESPACE:?}` 가드의 parse 차단 메커니즘과 값 형식(GitHub owner 소문자) 을 명시. (3) `docs/operations/alerting.md` §5.2 헬스체크 운영 절차에 주석 블록 추가 — `docker compose ps` 사용 전 `IMAGE_NAMESPACE` 확인 + compose parse 우회용 `docker inspect --format` fallback 을 명시. **서버 적용** (운영자 수동, 본 커밋과 분리): `~/aqts/.env` 에 `IMAGE_NAMESPACE=sucheol9773` 한 줄 추가. 저장소에 커밋되지 않으며, 비밀 정보가 아니므로 CI 에도 흘리지 않는다. **회고**: 본 이슈는 cosign keyless 레지스트리 namespace 도입 시점부터 운영 환경 `.env` 에 키가 없이도 "backend/scheduler 컨테이너 재기동 시에만" 실패하는 잠복 상태였다. 대부분의 운영 명령이 `docker compose up -d <service>` 형태여서 `prometheus`/`alertmanager` 단독 재기동이 드물었기 때문에 오랫동안 발견되지 않았다. 관측 공백(healthcheck) 을 메우는 순간 관측 도구 자체(compose ps) 가 다음 병목이 되는 전형적 패턴이며, `.env.example` 에 필수 키가 누락된 것이 파생 원인이었다 — 여섯 번째 "정의(docker-compose.yml 에 변수 참조) ≠ 적용(.env.example / 운영 .env 에 실제 값 공급)" 사례.
- [2026-04-15] P1-공급망: grype CVE DB 드리프트 대응 — Python 3.11 인터프리터 신규 won't-fix 2건 화이트리스트 등록 (`5bb0092` 문서-only 커밋 직후 CI Docker Build & Sign 잡의 grype 스캔이 `fail-build: true severity-cutoff: high` 로 차단된 건을 해소). 로그(`List High/Critical CVEs (grype, debug)` 스텝)에서 확인된 2건은 모두 `python 3.11.15 (no-fix)` — (a) **CVE-2026-6100** (Critical, CVSS 9.1, 2026-04-13 공개): `lzma.LZMADecompressor` / `bz2.BZ2Decompressor` / `gzip.GzipFile` 인스턴스 재사용 시 `MemoryError` 이후 use-after-free. 일회성 헬퍼(`lzma.decompress()` 등) 는 미영향. (b) **CVE-2026-4786** (High, CVSS 7, 2026-04-13 공개): `webbrowser.open()` 의 CVE-2026-4519 완화책 우회 (URL 에 "tion" 포함 시 shell command injection). **영향 감사** (직접 grep): `backend/` 에서 `import lzma` / `import bz2` / `import gzip` / `import webbrowser` 전부 zero matches, `LZMADecompressor` / `BZ2Decompressor` / `GzipFile(` / `webbrowser.open` 도 zero matches. 서버사이드 컨테이너는 네 모듈 중 어느 것도 직접 사용하지 않으며, `webbrowser` 는 데스크탑 API 로 런타임 무관. transitive 의존이 `lzma`/`bz2`/`gzip` 을 내부적으로 사용할 가능성이 있으나, Decompressor 인스턴스 재사용 + `MemoryError` 포착·무시라는 특수 실행 패턴이 필요해 현실 공격 표면이 무시할 만함. **결정**: 두 건 모두 fix 버전이 미출시이므로 `.grype.yaml` 의 "Python 3.11 인터프리터 won't-fix" 섹션에 만료일 `2026-06-06` 과 함께 등록 (기존 CVE-2026-4519 / CVE-2025-13836 와 동일 코호트). 근본 해소는 3.11 → 3.12 메이저 업그레이드 PR 에서 일괄 처리 (torch / stable-baselines3 호환성 검증 필요, 별건). 파이썬 CVE 검색 출처: `https://nvd.nist.gov/vuln/detail/CVE-2026-6100`, `https://nvd.nist.gov/vuln/detail/CVE-2026-4786`. **재발 방지**: grype DB 드리프트는 pinned 베이스 이미지 + rolling tag 조합의 구조적 특성이므로 향후에도 발생 가능. `.grype.yaml` 의 만료일을 수동 갱신하지 않으면 자동 무효화되어 CI 재차단 — 만료 시점에 일괄 재평가.
- [2026-04-15] P0-5 후속 (P0 승격): TradingGuard 교차프로세스 싱글톤 분열 발견 → Redis-backed state 이행 착수 — §10.17 의 `/api/system/kill-switch/*` 엔드포인트 도입 직후, 2026-04-16 11:30 KST Path A 집행(DEMO) 을 앞둔 dry-run 에서 P0 결함을 확인했다. **관측**: `docker exec aqts-scheduler python -c "g=get_trading_guard(); g.activate_kill_switch('DIAG'); print(g._state.kill_switch_on)"` → `True` (scheduler 프로세스의 subprocess 가 자기 싱글톤을 on 으로 전이). 같은 시점 backend 프로세스 `curl /api/system/kill-switch/status` → `kill_switch_on=false, kill_switch_reason=""`, `/metrics` → `aqts_trading_guard_kill_switch_active 0.0`, `/kill-switch/deactivate` → `was_on=false, previous_reason=""`. 두 프로세스가 **별개 TradingGuard 싱글톤**을 보유하고 있으며 서로의 상태 변경을 관측·제어하지 못함이 확인됐다. **근본 원인**: `backend/core/trading_guard.py:401-408` 의 `_guard_instance: Optional[TradingGuard] = None` 은 **프로세스 전역 in-memory 싱글톤**으로 설계됐으나, 현재 배포는 `docker-compose.yml` 에서 `backend` (`SCHEDULER_ENABLED=false`, HTTP API) 와 `scheduler` (`SCHEDULER_ENABLED=true`, `ReconciliationRunner`/`handle_midday_check`) 를 별도 컨테이너로 기동한다. `backend/main.py:234-235` 가 backend 에서 scheduler 를 비활성화하고, `backend/core/reconciliation_runner.py:71,120` 이 scheduler 프로세스의 로컬 guard 에 kill switch 를 활성화한다. 두 싱글톤은 메모리 공유가 없으므로 §10.17 엔드포인트는 scheduler 가 활성화한 kill switch 를 **관측도 해제도 할 수 없다**. **테스트 갭**: `backend/tests/test_kill_switch_routes.py` 9건은 단일 프로세스 `TestClient` 환경이라 정의상 이 결함을 재현할 수 없었다 — 알림 파이프라인 Wiring Rule 의 "정의 ≠ 적용" 원칙이 상태 공유 계층에 동일 적용된다. **결정**: 내일 11:30 Path A 집행을 취소하고 `docs/operations/midday-check-path-a-runbook.md` 를 v1.0.1 SUSPENDED 로 전환(§0.1 재개 조건 4건). Redis-backed `TradingGuard` 이행을 P0 로 승격하여 오늘 밤 ~ 내일 새벽 중 완료·배포·재예행을 목표로 한다. **범위**: (a) `_state` 필드(`kill_switch_on` / `kill_switch_reason` / 손실·드로다운 카운터 등) 를 Redis hash 로 이전 + pub/sub 채널(`aqts:trading_guard:state_change`) 로 프로세스 간 변경 브로드캐스트 + 각 프로세스의 in-process cache 는 구독으로 갱신. (b) Redis 장애 시 fail-closed — `activate_kill_switch` 는 로컬 차단 + 경보, `deactivate_kill_switch` 는 503 반환(감사 선행과 동일 정책). (c) 교차프로세스 통합 테스트를 `tests/integration/test_trading_guard_cross_process.py` 로 신설 — docker-compose 실행 환경에서 scheduler→backend / backend→scheduler 양방향 전파 실측 검증. (d) `/api/system/kill-switch/status` 는 Redis snapshot 기준으로 응답, `/kill-switch/deactivate` 는 감사 선행 후 Redis 단일 write + pub/sub publish. (e) 문서 갱신: `trading-halt-policy.md` §6 코드 연동 포인트에 Redis key·channel 추가, 본 §9 에 커밋 링크 append. 상세 증거·결정·후속 일정: `docs/operations/phase1-demo-verification-2026-04-11.md` §10.18. 일곱 번째 "정의(엔드포인트 HTTP 계약) ≠ 적용(공유 상태 조작)" 사례.
