...
**작성일**: 2026-04-10
**범위**: Commit 1 (state machine + retry API) → Commit 2 (Router wiring) → Commit 3 (retry loop + metrics + meta-alerts) 의 4 커밋 시리즈로 완성된 end-to-end 알림 파이프라인
...
## 5. NotificationRouter 와 채널 캐스케이드
...
router = NotificationRouter(
    primary=TelegramChannelAdapter(bot_token=..., chat_id=...),
    fallbacks=[
        FileNotifier(path="/var/log/aqts/alerts.jsonl"),
        ConsoleNotifier(),
    ],
)
alert_manager.set_notification_router(router)
```

`set_notification_router` 는 순수 setter — 주입 전에는 `_dispatch_via_router` 가 noop (로그만 남김). 이 구조 덕분에 단위 테스트는 모두 router 없이 실행되고, 통합 테스트와 운영에서만 실제 채널로 나간다.
...
### 6.2 왜 TradingScheduler 가 아닌 독립 task 인가
...
### 6.3 dispatch_retriable_alerts 알고리즘
