...
```bash
# 시스템 의존성 (최초 1회) — Graphviz `dot` CLI 만 필요.
brew install graphviz            # macOS
# sudo apt-get install graphviz  # Linux

# Python 의존성 — 표준 라이브러리 외 런타임 의존 없음.
# (이전 버전은 `pydeps` 를 썼지만, hop 필터가 전이 의존성을 잘라 아틀라스 누락
#  회귀가 있었음 → 제거. `backend/requirements-dev.txt` 내 lint 도구만 필요.)
...
`scripts/generate_diagrams.py` 가 `build_graph()` 의 AST 결과를 DOT 로 직렬화한 뒤 Graphviz `dot` 로 렌더한 전체 백엔드 그래프. 스캔된 **모든** `backend.*` 모듈이 노드로 들어가고 (홉 기반 필터 없음), 팀별 subgraph cluster 에 색상이 적용됩니다.

**용도**: 특정 모듈의 전역 위치와 전이 의존성 파악.

**`module-deps.overall.dot`** — 동일 그래프의 Graphviz DOT 소스. 스크립트 본문이 결정적으로 생성(노드/엣지 lexical 정렬)하므로 PR diff 가 가능하며, `--check` 는 DOT 를 바이트 비교합니다. SVG 는 Graphviz 폰트/버전에 따라 달라질 수 있어 `--check` 범위에서 제외.
...
커스텀 AST 패스는 **정적 import** 만 추적합니다. 다음 wiring 은 **보이지 않으므로** `wiring.*.mmd` 에서 수작업으로 보완:
...
CLAUDE.md §9 에 **ADR-004 Graphify Pilot** 이 Phase 2 외부 도구 심사 후보로 올라 있습니다. 본 디렉토리의 산출물은 **ADR-004 심사와 무관**하며, 표준 라이브러리 AST + Graphviz `dot` CLI + Mermaid (모두 OSS) 로만 구성되어 Graphify 평가를 선점하거나 편향시키지 않습니다.

ADR-004 가 Stage 2 심사를 통과할 경우, 본 산출물은 **비교 baseline** 으로 활용될 수 있습니다 — "Graphify 가 AST + Graphviz + 수작업 Mermaid 대비 어떤 부가가치를 제공하는가" 를 정량 평가하는 기준선. 다만 이는 ADR-001 §2.2 sandbox 프로세스의 판정 권한을 침해하지 않으며, 본 디렉토리는 Graphify 채택 여부와 독립적으로 유지·확장됩니다.
