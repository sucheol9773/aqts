...
- [ ] **black 포맷 drift 해소 (진행 중, 2/26 잔여)**: `python -m black --check` 기준 잔여 2 파일 reformat 대기. 2026-04-21 `chore/black-format-drift` 브랜치에서 진행 — 팀메이트 1(strategy_ensemble) + 팀메이트 2(scheduler/data_collector) + 팀메이트 3(alembic/api/ai_analyzer/idempotency/order_executor/portfolio_manager/db) 영역 해소 완료. 잔여 대상: `backend/tests/{test_opinion,test_system_routes}.py` (팀메이트 4). development-policies.md §9 "전수 처리 원칙" 적용.
