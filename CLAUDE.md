...
- **postgres WAL archive 권한 19일 silent miss (2026-04-26, OPS-028)**: `aqts_postgres_wal_archive` 볼륨이 인프라 셋업 시점부터 root:root drwxr-xr-x 로 생성되어 postgres archive_command 가 19일간 매분
  Permission denied. pg_wal/ 13.5GB 누적 → 디스크 98% → catch-up 폭주로 100% PANIC → mongodb docker network corruption cascade. 35분 P0 incident, 데이터 손실 0. 본질 = §14 Wiring Rule 이 1회성 인프라 셋업
  사각지대 → development-policies.md §14.5 신설. 작업 기록: `docs/operations/postgres-wal-archive-permission-2026-04-26.md` (OPS-028).
...
- [x] **pip-audit 하드코딩 해소 (완료, 2026-04-25)**: `chore/pip-audit-deps-split` 브랜치에서 옵션 A (별도 파일) 로 해소. `backend/requirements-security.txt` 신설(pip-audit==2.7.3 SSOT, `-r requirements.txt` 의도적 미포함 — OPS-018 동일 논리), `.github/workflows/ci.yml` 의 `Install pip-audit` 스텝(구 라인 94-95) 을 `Install security scanners` 로 리네임 + `pip install pip-audit==2.7.3` → `pip install -r backend/requirements-security.txt` 전환, `cache-dependency-path` 를 multi-line YAML literal 로 두 파일(`requirements-dev.txt` + `requirements-security.txt`) 모두 명시 (한 파일이라도 변경 시 cache invalidation 트리거 — silent miss 방어선). `requirements-dev.txt:32-33` "분리 예정" 코멘트 → "분리 완료, OPS-025" 갱신, README.md 프로젝트 트리에 `requirements-security.txt` 한 줄 추가. 옵션 B (`requirements-dev.txt` 보안 섹션 병합) 는 dev/security lifecycle 혼재로 미채택, 향후 bandit/safety 등 추가 시 자연스러운 확장을 위해 별도 파일 채택. 작업 기록: `docs/operations/security-deps-split-2026-04-25.md` (OPS-025).
- [x] **lxml 6.1.0 업그레이드 (CVE-2026-41066 후속, 완료 2026-04-25)**: `chore/lxml-6.1.0-upgrade` 브랜치에서 `backend/requirements.txt:71` 의 `lxml==5.2.2` → `lxml==6.1.0` 업그레이드 + 두 ignore 파일(`backend/.pip-audit-ignore`, `.grype.yaml`) 의 GHSA-vfmq-68hx-4jfw 동일 커밋 동시 삭제. Smoke test = BeautifulSoup 4.12.3 + lxml 6.1.0 + feedparser 6.0.11 격리 venv 에서 6 종 HTML fixture + 실 BBC RSS 3 entries end-to-end pull 통과. OPS-022 parity (grype=26→25, pip-audit=4→3, shared=4→3) + OPS-026 expiry (전량 보존) 정적 검사기 그린. 작업 기록: `docs/operations/lxml-6.1.0-upgrade-2026-04-25.md` (OPS-021). 직전 임시 ignore 도입은 `chore/pip-audit-ignore-lxml-xxe` 브랜치에서 `backend/.pip-audit-ignore` 에 `GHSA-vfmq-68hx-4jfw` 를 만료일 2026-06-06 으로 등록하여 CI 블록 우선 해소. 현재 앱 코드의 `iterparse`/`ETCompatXMLParser`/`etree.parse`/`etree.fromstring` 직접 호출은 0 건이며, 유일한 lxml 간접 사용(`backend/core/data_collector/news_collector.py:222/224` 의 `BeautifulSoup(raw, "lxml").get_text()`) 은 HTML 파서 경로로 취약 sink 외. 만료일 이전에 반드시 수행해야 할 후속 작업: (1) `backend/requirements.txt:71` 의 `lxml==5.2.2` → `lxml==6.1.0` 업그레이드, (2) `beautifulsoup4==4.12.3` 과 lxml 6.x 조합 smoke test — 특히 `news_collector._parse_entry` 의 RSS 본문 파싱이 깨지지 않는지 실측(`pytest tests/ -k news_collector` + 실 RSS feed 1~2개 수동 pull), (3) 업그레이드 후 `.pip-audit-ignore` **및 `.grype.yaml`** 의 GHSA-vfmq-68hx-4jfw 항목을 **동시에** 삭제, (4) CLAUDE.md §9 본 TODO 를 [x] 로 전환하고 retrospective 문서 필요 시 `docs/operations/lxml-6.1.0-upgrade-<date>.md` (OPS-021) 로 기록. 우선순위: 만료일 2주 전(2026-05-23) 까지 완료 목표. silent miss 방어: 만료일 이전에 누군가 `iterparse` 또는 `ETCompatXMLParser` 를 새로 도입하면 본 ignore 의 "코드 경로 미사용" 전제가 깨지므로, 만약 향후 해당 sink 사용이 필요해지는 PR 이 생기면 그 PR 이 곧바로 lxml 업그레이드 PR 로 합쳐져야 한다. **Silence Error 재발 방지 (2026-04-22 회고)**: `chore/pip-audit-ignore-lxml-xxe` 커밋 당시 `backend/.pip-audit-ignore` 만 업데이트하고 `.grype.yaml` 병기 엔트리를 누락하여, PR #25 머지 후 main CI 의 `anchore/scan-action@v6` 스텝이 lxml GHSA-vfmq-68hx-4jfw 를 다시 high 레벨로 차단한 회귀 발생. `fix/grype-yaml-glibc-lxml-parity` 브랜치에서 .grype.yaml 병기 추가로 해소. 교훈 — **컨테이너 스캐너(grype)와 파이썬 의존성 스캐너(pip-audit)는 ignore 목록을 공유하지 않는다.** 신규 CVE/GHSA 억제 시 `backend/.pip-audit-ignore` 와 `.grype.yaml` 을 **동일 커밋에서** 병행 업데이트 필수. 후속 정적 검사기 후보 TODO 로 `scripts/check_vuln_ignore_parity.py` 신설 예정.
- [x] **.grype.yaml ↔ backend/.pip-audit-ignore parity 정적 검사기 신설 (완료, 2026-04-23)**: `chore/check-vuln-ignore-parity` 브랜치에서 `scripts/check_vuln_ignore_parity.py` 신설. 두 파일의 CVE/GHSA 식별자 집합을 stdlib 정규식 기반 파서로 대조하여 상호 배타적 차집합이 존재하면 error, `grype-only`/`pip-audit-only` 라인 레벨 키워드 마커로 의도된 단방향 예외 허용. Doc Sync 워크플로에 `Run vuln-ignore parity check` 스텝 추가. 기존 `.grype.yaml` 의 21 개 OS/Python CVE 엔트리 인라인 주석에 `grype-only` 키워드 병합하여 기본 상태 정합화 (grype=25, pip-audit=4, shared=4 — parity OK). 테스트 하니스 15 tests 추가 (Stage 2/3 와 동일 6 그룹 구조: 동일 집합 통과 / 단방향 에러 / 마커 허용 / 파서 오탐 방지 / 실제 레포 회귀 / main() 진입). 작업 기록: `docs/operations/check-vuln-ignore-parity-2026-04-23.md` (OPS-022). 2026-04-22 lxml GHSA-vfmq-68hx-4jfw silent miss 회귀 (PR #25 머지 후 main CI grype 가 high 로 재차단) 에 대한 구조적 방어선 — 이후 신규 CVE/GHSA 억제 시 두 파일 동시 업데이트 혹은 단방향 마커 명시가 CI 강제.
- [x] **화이트리스트 만료일 정적 검사기 신설 (완료, 2026-04-23)**: `chore/check-vuln-ignore-expiry` 브랜치에서 `scripts/check_vuln_ignore_expiry.py` 신설. `.grype.yaml` 과 `backend/.pip-audit-ignore` 의 각 엔트리에서 `YYYY-MM-DD` 만료일을 파싱하여 오늘(UTC) 이전이거나 결손이면 CI error. Doc Sync 워크플로 parity 스텝 직후 `Run vuln-ignore expiry check` 스텝 추가. 기존 반응형 경로(만료 당일 pip-audit --strict 재노출 → CI 차단 → 역추적)를 선제적 차단(만료일 이전 PR 레벨 고지)으로 전환. `check_expiry(entries, today)` 시그니처로 기준일을 테스트에서 주입 가능하게 설계 — 13 tests / 6 그룹 (유효 / 만료·결손 / 경계 / 파서 오탐 / 실제 레포 / main). 현 상태: `grype=26, pip-audit=4, reference=2026-04-23 UTC` PASS (2026-06-06 만료 엔트리 전량). 작업 기록: `docs/operations/check-vuln-ignore-expiry-2026-04-23.md` (OPS-026). PR #38 (CVE-2026-3298 hotfix) 직후 신설 — 공급망 보안 레이어 4축(집합 비대칭 / 만료 / 형식 오류 / 신규 CVE) 중 "만료" 축 담당.
...
- [x] **ADR-002 anthropic-skills Stage 1 Proposal (완료, 2026-04-22, PR #24)**: `docs/adr-002-anthropic-skills` 브랜치. Pilot = 팀메이트 4 (Tests/Doc-Sync), 교체 지정자 = 팀메이트 1. Scope 는 `anthropic-skills:` prefix 공식 스킬로 한정 (커뮤니티 스킬 = ADR-006 분리). ADR-001 §2.2 Stop 조건 3종 + 4번째 "Pilot 14일 내 참조 < 5회 → 팀 1 교체 후 재시작" 확장. 실패 모드 5개 + 감지 방법 명시.
- [x] **Cowork → 4-세션 migration Phase 1~4 (완료, 2026-04-22, PR #28 머지)**: `chore/multi-session-migration` 브랜치에서 4 Phase 순차 이행. Phase 1 = `../aqts-team{1,2,3}-<role>` worktree 3개 신설 (팀 4 pilot 기존 유지); Phase 2 = `.claude/settings.json` (permissions.deny 에 §2.5 리드 전용 6 경로 + Bash force-push/hard-reset/rm-rf, env.PYTHONUNBUFFERED=1) + `.gitignore` 에 `.claude/settings.local.json` 추가; Phase 3 = `scripts/team/` 5 스크립트 (`bootstrap_worktree.sh`/`teardown_worktree.sh`/`mailbox_new.sh`/`wiring_smoke.sh`/`pre_bash_guard.sh`) + `agent_docs/mailboxes/` + PreToolUse hook 주입으로 Wiring Rule 완성, 팀 4 ADR-002 Pilot lockout (2026-05-06 까지); Phase 4 = `.mcp.json` (`github` 1개 opt-in, GCP MCP 는 단일 VM + SSH 체제라 제외). 작업 기록: `docs/operations/claude-multisession-migration-2026-04-22.md` (OPS-023) + `docs/operations/mcp-setup-2026-04-22.md` (OPS-024). `agent_docs/governance.md §1` harness 표 동반 갱신 (4-세션 방식 명시). 후속 리드 작업 = Pilot `.claude/settings.local.json` 로컬 override (`"permissions": {"allow": ["*"]}` + `"disabledMcpjsonServers": ["*"]`) + 팀메이트 PAT 배포 가이드 공지.
- [ ] **ADR-002 Stage 2 Sandbox 관찰 (진행 중 2026-04-22 ~ 2026-05-06)**: `docs/adr-002-stage2-kickoff` 브랜치에서 kickoff 개정 (삼중 SSOT = `anthropics/skills` 구현체 17 디렉토리 + `agentskills/agentskills` 스펙 + `skills-ref` SDK, LICENSE PASS = Apache-2.0 (code) + CC-BY-4.0 (docs), Adoption PASS = `agentskills.io/clients` 37+ 벤더, 실패 모드 F6 (Gotchas 누락) / F7 (SKILL.md 500 줄 초과) 확장, Stage 2→3 Exit Criteria 정량화 = `delta.pass_rate ≥ 0.2` / tokens ≤ 2x / 20-query trigger eval 60/40 split, Pilot 온보딩 §7.5 신설). **Stage 2 Exit 판정일 = 2026-05-06** — `docs/architecture/sandbox/adr-002/skill-usage-log-W{1,2}.md` 주간 로그 수집 + §7.2 표 업데이트. **판정 경로 — 세 게이트 모두 ADR-002 §5.3.1 을 단일 진실원천으로 집행한다 (본 요약과 충돌 시 §5.3.1 우선)**: (A) **정량 Output quality** — `delta.pass_rate ≥ 0.2` AND `tokens_delta ≤ 2.0x`, (B) **정량 Trigger accuracy** — 20-query eval / 3 runs / validation pass rate ≥ 0.7, (C) **정성 관찰** — 참조 ≥ 5회 + F1~F7 미발현. **전환 결정**: (A) + (B) + (C) 모두 PASS → Stage 3 진입; (C) 에서 참조 < 5회 (단 F1~F7 미발현) → 팀메이트 1 로 Pilot 교체 + Stage 2 재시작; F1~F7 중 1건 이상 발현 또는 (A)/(B) FAIL → Stop + ADR Status=Rejected.
- [ ] **OPS-028 후속 — wal_archive 자동 chown (P0, 발견 2026-04-26)**: `docker-compose.yml` postgres entrypoint override 또는 init container 로 wal_archive `chown postgres:postgres + chmod 700` 자동화. 현재 hotfix (`docker exec -u root
chown`) 는 컨테이너 인스턴스 한정 — 재생성 / 볼륨 재신설 시 회귀. 팀 2 위임 메일: `agent_docs/mailboxes/team2/inbox/20260426-0014-wal-archive-auto-chown.md`. 응답 deadline 2026-04-29.
- [ ] **OPS-028 후속 — pg_stat_archiver 알림 신설 (Ask, 발견 2026-04-26)**: postgres_exporter 도입 + `aqts_postgres` prometheus group (`AqtsPgArchiveFailing`, `AqtsPgArchiveStale`, `AqtsPgWalSizeHigh`). 본 사건은 *증상* (디스크) 만 alert 하고
*원인* (archive 실패) 은 detection 0 였음. 팀 2 위임 메일: `agent_docs/mailboxes/team2/inbox/20260426-0014-pg-stat-archiver-alert.md`. 머지 deadline 2026-05-06.
- [x] **OPS-028 후속 — boot disk 48GB → 100GB 증설 (완료, 2026-04-27)**: `aqts-server` boot disk GCP 측 online resize (`gcloud compute disks resize aqts-server --size=100GB --zone=asia-northeast3-a` Mac 본인
IAM) → 서버 측 partition + filesystem 확장 (`growpart /dev/sda 1` + `resize2fs /dev/root`). `df -h /` 확인: 47G → 96G, 사용률 55% → 28%, 가용 70G. 본 작업이 `development-policies.md §14.5` 운영 Wiring Rule 의
첫 self-application 사례 — invariant 1 (setup-time `df -h` 확인) ✓, invariant 2 (기존 `AqtsDiskUsageHigh/Critical` 알림이 새 base 96GB 의 80%/90% = 76.8GB/86.4GB 로 자동 추적) ✓. invariant 3 (self-documenting)
은 본 §9 entry + OPS-028 회고 §6.3 reference 로 충족.
- [ ] **운영 Wiring Rule §14.5 후속 — 팀 2 + 팀 4 implementation (Ask, 발견 2026-04-26)**: §14.5 정책 머지 후 (1) 팀 2 가 `docs/operations/infrastructure-setup-checklist.md` 신설 + 12 docker volume / prometheus job / alertmanager receiver
  backfill audit, (2) 팀 4 가 `scripts/check_infra_invariants.py` 5 카테고리 (volume permissions / network IP / external reach / pg_stat_archiver / log patterns) + 18+ tests + GHA scheduled. 위임 메일:
  `team2/inbox/20260426-0028-infrastructure-setup-discipline.md`, `team4/inbox/20260426-0028-infrastructure-invariant-scanners.md`. 머지 deadline 2026-05-13 ~ 2026-05-20.
---
