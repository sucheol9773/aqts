...
- [ ] **.grype.yaml ↔ backend/.pip-audit-ignore parity 정적 검사기 신설 (발견 2026-04-22)**: lxml GHSA silent miss 재발 방지 수단. `scripts/check_vuln_ignore_parity.py` 를 신설하여 두 파일에서 CVE/GHSA 식별자 집합을 추출하고, **상호 배타적 차집합이 존재하면 error** 로 판정하는 정적 검사기. 단, 한쪽에만 존재하는 것이 **의도된** 경우(예: Python 전용 패키지는 grype 가 감지 못 함, OS 패키지는 pip-audit 가 감지 못 함) 를 위해 `# grype-only`/`# pip-audit-only` 주석 마커로 예외 허용. Doc Sync 워크플로에 0 errors 강제 등록. 위험도 낮음, 예상 수정 범위 2 파일 (스크립트 신설 + workflow 1 스텝 추가). 작업 기록 위치: `docs/operations/check-vuln-ignore-parity-<date>.md` (OPS-022).
