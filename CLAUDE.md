...
- [ ] **AST 기반 정적 검사기 커버리지 확장 (진행 중, `check_bool_literals.py` 완료 2026-04-22)**: `chore/check-bool-literals-ast` 브랜치에서 `scripts/check_bool_literals.py` 의 Python 검사부를 regex → AST 로 전환 + `backend/tests/test_check_bool_literals.py` (27 tests) 추가. regex 4대 결손(중첩 괄호·멀티라인·문자열 리터럴 false positive·비교 순서 역전) 을 AST 노드 판정으로 구조적 해소. `ast.Compare` (Eq/NotEq/In/NotIn 양방향) + `ast.Call` `.lower()` 체인 + `ast.Tuple/List/Set` 컨테이너 판정. 컨테이너 판정 술어는 `_BOOL_LITERAL_TOKENS` (`env_bool()` 허용 토큰과 동치) 으로 제한하여 `os.getenv("APP_ENV") in ("prod", "staging")` 같은 enum-style 멤버십은 통과 (Codex P2 회귀 방어). 면제 파일(`backend/core/utils/env.py`, 자기 자신) 로직 유지. 설정 파일 KV-regex 경로는 AST 파싱 대상 아니므로 스코프 밖. 작업 기록: `docs/operations/check-bool-literals-ast-2026-04-22.md` (OPS-019). 잔여: `check_rbac_coverage.py` 테스트 하니스(Stage 3).
