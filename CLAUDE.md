...
- [ ] **ADR-002 anthropic-skills Stage 1 Proposal (작성 완료 2026-04-22, 리드 승인 대기)**: `docs/adr-002-anthropic-skills` 브랜치에서 `docs/architecture/adr-002-anthropic-skills-adoption.md` 신설. Pilot = 팀메이트 4 (Tests/Doc-Sync), Pilot 교체 지정자 = 팀메이트 1. 핵심 결정: (1) Scope 는 `anthropic-skills:` prefix 공식 스킬로 한정, 커뮤니티 스킬은 ADR-006 으로 분리, (2) ADR-001 §2.2 Stop 조건 3종 + 4번째 "Pilot 14일 내 실제 참조 < 5회 → 팀메이트 1 교체 후 재시작" 확장, (3) §2.3 평가표에서 라이선스 확인만 Pending — 리드가 머지 전 `anthropic/skills` LICENSE 직접 확인 필요, (4) Stage 2~4 타임라인 D~D+58. 실패 모드 5개 + 감지 방법 명시. **리드 확인 필요 (Stage 1 → 2 전환)**: §5.2 의 1~3 항목 (LICENSE 확인, prefix 스냅샷 승인, Pilot worktree 생성 일정). 완료 시 본 TODO 를 [x] 로 전환하고 Stage 2 Sandbox 관찰 기록 주간 업데이트 시작.
