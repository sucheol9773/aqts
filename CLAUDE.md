...
- [ ] **Dev 의존성 파일 분리 (발견 2026-04-21)**: 현재 `ruff==0.5.0` + `black==24.4.2` 가 `.github/workflows/ci.yml` 라인 39 에 하드코딩되어 있어 로컬 개발자가 lint 도구 설치 방법을 찾기 어렵고 (CLAUDE.md §1 "하드코딩 금지" 규칙과 미묘한 긴장), 버전 업그레이드 시 CI yml 을 건드려야 하는 번거로움. 개선 방향 — **옵션 A**: `backend/requirements-dev.txt` (`-r requirements.txt` + ruff/black/pre-commit 등) 신설하고 CI 는 `pip install -r requirements-dev.txt` 로 단순화, **옵션 B**: `backend/pyproject.toml` 의 `[project.optional-dependencies].dev` (PEP 621) 채택 후 `pip install -e ".[dev]"` 로 설치. 본 이슈는 `chore/python-version-align` 진행 중 venv 초기 셋업 과정에서 발견됨. 위험도 낮음, 예상 수정 범위 3-4 파일.
