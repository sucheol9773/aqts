...
전체 pytest 를 실행할 때는 `timeout` wrapper 의 값을 **최근 관측된 러너 시간보다 최소 1.5배 이상**으로 설정한다. 직전 관측 기준선(2026-04-21): 4134 passed 기준 약 318s, 따라서 최소 540s 를 권장한다 (1.5배 = 477s 이므로 540s 가 여유있는 상한). 러너 시간은 테스트 수 증가에 따라 변동하므로, 타임아웃에 걸리면 먼저 실제 러너 시간을 관측한 뒤 다음 실행의 타임아웃 값을 조정한다. "일단 짧게 주고 실패하면 늘린다" 패턴은 불필요한 재실행 비용이다.
...
- **정적 검사기 경로 스캔 인프라 미통일 silent miss**: 각 `scripts/check_*.py` 가 개별적으로 `rglob("*.py")` + 인라인 필터를 구현하면, 하나의 검사기가 `__pycache__` 만 제외하고 `.venv` / `site-packages` / `build` 를 빠뜨려도 다른 검사기는 알아차릴 수단이 없다. 결과는 두 방향으로 나타난다: (1) 서드파티 코드가 프로젝트 규칙 위반으로 오탐되어 운영자가 "false positive 많으니 끄자" 결론에 도달 — 방어선이 무력화, (2) 대용량 vendored 트리 AST 파싱으로 CI/샌드박스 타임아웃 — 방어선이 실행되지 않음. 두 경우 모두 "검사기는 등록되어 있으나 실질 차단력은 없는" 상태로 수렴한다. **대응**: 경로 스캔 단일 진입점 `scripts/_check_utils.py::iter_python_files` 를 SSOT 로 사용한다. `VENDORED_DIR_NAMES` 에 `.venv` / `venv` / `__pycache__` / `.tox` / `build` / `dist` / `.pytest_cache` / `htmlcov` / `.mypy_cache` / `.ruff_cache` / `node_modules` / `site-packages` 를 등록하고, 내부 구현은 `os.walk` 의 `dirnames[:]` 슬라이스 대입으로 서브트리 재귀를 **사전 차단** 한다 (`rglob` 후 필터는 대용량 venv 로 이미 내려간 뒤이므로 부적절). 신규 `check_*.py` 는 인라인 `rglob`/`glob` 을 금지하고 반드시 `iter_python_files(root)` 를 경유한다. 회귀 사례 (2026-04-21): `check_bool_literals.py` 가 `backend/.venv/` 를 스캔해 226 false positives, `check_loguru_style.py` 가 동일 경로 AST 파싱으로 15 s 샌드박스 타임아웃. 상세: `docs/operations/static-checker-venv-audit-2026-04-21.md`.
...
### 14.5 운영 Wiring Rule — 1회성 인프라 셋업의 "정의 ≠ 적용"

  §14.1~14.4 의 Wiring Rule 은 *PR 단위 코드 변경* 에 적용된다. 그러나 다음 1회성 인프라 작업도 동일 패턴의 silent miss 위험을 보유한다:
  ...
  이런 작업은 "코드 PR 이 아니므로 §14.1~14.4 가 안 잡는다" 의 사각지대다. **OPS-028 사건 (postgres WAL archive 19일 silent miss)** 이 직접 사례 — `aqts_postgres_wal_archive` 볼륨이 인프라 셋업 시점부터
  root:root drwxr-xr-x 로 생성되어 postgres 가 archive cp 를 19일간 매분 Permission denied 로 실패. `pg_wal/` 가 13.5GB 까지 누적되며 디스크 98% 도달 후 cascade.

  본 §14.5 는 다음 3개 invariant 를 1회성 인프라 작업에 강제한다:

  1. **Setup-time verification** — 작업 직후 *작동 검증* 을 명시 수행. 단순 "생성 완료" 가 아니라 "의도된 사용 경로의 end-to-end 흐름" 을 확인. 예 (OPS-028 회피): wal_archive 볼륨 신설 직후 postgres 가 실제 cp
  성공하는지 1회 trigger 후 `pg_stat_archiver.archived_count > 0` 확인.

  2. **Continuous verification** — 시간이 지나도 invariant 가 유지되는지 *지속* 확인하는 매트릭/스크립트/알림 중 최소 1개 채널 보유. set-and-forget 금지.
     - 매트릭 채널: prometheus exporter (postgres_exporter, mongodb-exporter 등)
     - 스크립트 채널: `scripts/check_*.py` 정기 실행 (cron, GHA scheduled, systemd timer)
     - 알림 채널: 매트릭/스크립트 결과가 임계 초과 시 alertmanager 트리거

  3. **Self-documenting setup** — 셋업 절차가 `docs/operations/` 의 reusable checklist 또는 runbook 으로 기록. "메일박스만 보면 무엇을 했는지 추적 가능" 한 audit trail.

  **강제 적용 시점**:
  - 신규 인프라 작업 시: **시작 전** 본 §14.5 의 3 invariant 가 어떻게 충족되는지 PR 본문 (IaC 라면) 또는 OPS 회고 문서 (수동 작업이라면) 에 명시.
  - 기존 인프라 작업의 backfill: `docs/operations/infrastructure-setup-checklist.md` (팀 2 작성) 를 통해 모든 기존 invariant 를 audit.

  **회귀 방어선**: §14.1~14.4 와 마찬가지로 본 §14.5 의 violation 사례를 `CLAUDE.md §5` 에 누적 기록하여 학습 트레일을 유지한다. 향후 silent miss 시리즈 #8 이 본 정책 누락으로 발생하면 §14.5 의 적용 범위를 다시
  확대해야 한다.
