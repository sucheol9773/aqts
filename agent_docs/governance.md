...
| 우선순위 | SLA | 미응답 시 조치 |
|---|---|---|
| `[P0]` | 4시간 | 같은 세션 내 즉시 응답. 초과 시 리드가 직접 개입 |
| `[Ask]` | 24시간 | 초과 시 리드에게 에스컬레이션 |
| `[Lead-Approval]` | 48시간 | 초과 시 리드가 주간 싱크에서 우선 처리 |
| `[FYI]` | 없음 | 응답 불필요. 읽었으면 `processed/` 로 이동 |

SLA 준수 확인: `bash scripts/team/mailbox_check_sla.sh [team_number|all]`. 각 팀메이트는 세션 시작 시 자기 inbox 를 확인하고, SLA 초과 메시지가 있으면 우선 처리한다.
