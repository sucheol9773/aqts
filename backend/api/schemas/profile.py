...
class ProfileUpdateRequest(BaseModel):
    ...
    investment_style: str = Field(
        ...,
        description="투자 스타일 (DISCRETIONARY, ADVISORY)"
    )
    investment_goal: str = Field(
        ...,
        description="투자 목적 (WEALTH_GROWTH, RETIREMENT, EDUCATION, INCOME)"
    )
...
class ProfileResponse(BaseModel):
    ...
    created_at: Optional[str] = Field(
        default=None,
        description="프로필 생성 시간 (ISO 8601)"
    )
    updated_at: Optional[str] = Field(
        default=None,
        description="프로필 수정 시간 (ISO 8601)"
    )
