...
class ModeManager:
    ...
    def check_backtest_to_demo(self) -> TransitionCheckResult:
        ...
        # 상태 판정
        status = TransitionStatus.READY if all(i.passed for i in items if i.required) else TransitionStatus.NOT_READY
    ...
    def check_demo_to_live(self) -> TransitionCheckResult:
        ...
        items.append(
            TransitionCheckItem(
                name="리스크 한도 설정",
                passed=(risk.daily_loss_limit_krw > 0 and risk.max_drawdown > 0 and risk.max_order_amount_krw > 0),
                message=(
